...
\begin{enumerate}
    ...
    \item \textbf{Median Relative Error:} For each experimental run, we calculate the mean relative error across all identifiable parameters. The relative error for a single parameter $p$ is defined as $|p_{\text{true}} - p_{\text{estimated}}| / |p_{\text{true}}|$. Runs that failed to produce any result are assigned a penalty error of $10^6$. We then report the median of these per-run mean errors across all experiments. We use the median because it is robust to occasional outliers. For locally identifiable systems where our method returns multiple solutions, for the purpose of this benchmark we select the solution closest to the known ground truth to calculate the error.
\end{enumerate}
...
The results in Table~\ref{tab:overall_performance} highlight several key findings. Our GPR-enhanced method, \texttt{ODEPE-GPR}, demonstrates excellent reliability, with a high success rate even at a strict 1\% error threshold (SR-1). While the \texttt{SciML} optimizer achieves a higher success rate at the looser 50\% threshold, our GPR-enhanced method is more likely to produce a high-precision result. In terms of accuracy, \texttt{ODEPE-GPR} achieves a median error of 0.21\%, which is highly competitive.
